...
const getUserRegisteredProgram = (userId, callback) => {
    const sql = 'SELECT p.programId, p.programTitle, p.startDate, p.endDate, TIME_FORMAT(p.startTime, "%h:%i %p") AS startTime, TIME_FORMAT(p.endTime, "%h:%i %p") AS endTime, p.location, p.image, rp.registrationdate FROM registeredprogram rp JOIN program p ON rp.programId = p.programId WHERE rp.userId = ?';
